...
// ---------- عناصر مساعدة للعرض ----------
function statusColor(pct, goodMax, warnMax) {
...
function renderOverview() {
  ...
  kpiGrid.innerHTML = '';
  const kpis = [
    { label: 'نقطة التعادل اليومية', value: fmt(m.breakEvenDaily), note: 'المبيعات اللازمة يومياً لتغطية كل التكاليف' },
    { label: 'الربح / الخسارة الشهري المتوقع', value: fmtSigned(m.avgScenario.profit), note: `عند متوسط مبيعات ${fmt(m.salesAvg)} يومياً`, tone: m.avgScenario.profit >= 0 ? 'good' : 'critical' },
    { label: 'نسبة تكلفة المواد من الإيرادات', value: m.totalCogsPct.toFixed(0) + '%', note: 'المعدل الصحي المستهدف: 35–40%', tone: statusColor(m.totalCogsPct, 40, 55) },
    { label: 'هامش الربح لكل كيلو دجاج', value: fmt(m.marginPerKg), note: 'الفرق بين إيراد الكيلو وتكلفة شراء الدجاج فقط' },
  ];
  kpis.forEach((k) => {
  ...
  const scenarioWrap = document.getElementById('scenario-cards');
  scenarioWrap.innerHTML = '';
  const scenarioMeta = {
    Low: { label: 'يوم ضعيف', sub: `${fmt(state.salesLow)} يومياً` },
    Avg: { label: 'يوم متوسط', sub: `${fmt(m.salesAvg)} يومياً` },
    High: { label: 'يوم قوي', sub: `${fmt(state.salesHigh)} يومياً` },
  };
  m.scenarios.forEach((sc) => {
    const tone = sc.profit >= 0 ? 'good' : 'critical';
    const card = el('div', 'card scenario-card');
    card.innerHTML = `
      <div class="scenario-title">${scenarioMeta[sc.key].label}</div>
      <div class="scenario-sub">${scenarioMeta[sc.key].sub}</div>
      <div class="scenario-row"><span>الإيراد الشهري</span><b>${fmt(sc.monthlyRevenue)}</b></div>
      <div class="scenario-row"><span>إجمالي التكاليف</span><b>${fmt(m.totalMonthlyCosts)}</b></div>
      <div class="scenario-row scenario-result tone-${tone}"><span>${sc.profit >= 0 ? 'الربح' : 'الخسارة'}</span><b>${fmtSigned(sc.profit)}</b></div>`;
    scenarioWrap.appendChild(card);
  });

  // مدخلات المبيعات والإنتاج
